...
def get_borders(
...
):  # return borders of image, usually in format: first row, first column, last row, last column => fr, fc, lr, lc
    if not reverse:
        return (
            matrix[0, :],
            matrix[:, 0],
    ...
    return (
        ...
        matrix[0, :],
        matrix[:, 0],
    )  # when finding new image, we need borders reversed => lr, lc, fr, fc


def find_first(
    dicto,
):  # find first corner image - from it we will add neoghbouring images
                        ...
                        for second_index, second_border in enumerate(neighbour_borders):
                            if (first_border == second_border).all():
                                neigbhours.append(
                                    (
                                        first_image,
                                        pointings[first_index],
                                        pointings[second_index],
                                    )
                                )
...
def set_first_positions(
    first_pointing, second_pointing
):  # set correct position of first image according to flip and rotation
    fr, fc = None, None
...
def find_new_position(
    x, y, r
):  # find position of new image according to old image position and touching side
    if r == 0:
        return x - 1, y
    if r == 1:
        return x, y - 1
...
def same(
    line, loch_line, indeces=[]
):  # find if is monster present on this line; possibly should obey found already indeces
    positions_indeces = []
    if (
        not indeces
    ):  # if we are not fixed on some indeces (because of already found indeces of "upper monster body")
        indeces = [
            start for start in range(len(line) - len(lochness[0]))
        ]  # just create all possible indeces
    for start in indeces:
        loch_pos = 0
        for i in range(start, start + len(lochness[0])):  # find that monster!
            if (line[i] == "#" and loch_line[loch_pos] == "O") or (
                loch_line[loch_pos] == "."
            ):
                loch_pos += 1
...
def get_res(image):  # find all monsters for this possible image
    ...
    whole_image = image
    indeces = [
        (a, a + 1, a + 2) for a in range(len(whole_image) - 2)
    ]  # all possible triplet for monster line indeces
    for i, j, k in indeces:
        ...
        if positions:  # we have to keep up with column index where monster(s) started!
            ...
            if positions:  # same, should keep indeces
                ...
                lochs_length += 15 * len(
                    positions
                )  # if monster(s?) found, add their length
    return lochs_length  # return found monster spaces


flip_rotates = defaultdict(
    list
)  # make all possible rotations and flips and save them to map
...
RESOLUTION = int(
    sqrt(len(flip_rotates))
)  # save length of image (both horizontal and vertical are same, it's a square)
...
while (
    queue
):  # find positions of all images(maybe in some flipped/rotated order but correct!):
    existing_index = queue.pop()  # pop last added image from queue
    (row_index, column_index), first_image = images_positions[
        existing_index
    ]  # get his coordinates...
    ...
    for key, new_images in flip_rotates.items():  # search for new neighbour
        ...
        for new_image in new_images:  # take some new image
            new_images_borders = get_borders(
                new_image, reverse=True
            )  # get his borders in special order
            for first_index, first_border in enumerate(first_image_borders):
                ...
                for second_index, second_border in enumerate(new_images_borders):
                    ...
                    ):  # compare only logical borders (according to ordering)
                        if (
                            first_border == second_border
                        ).all():  # found neighbour! (possibly)
                            x, y = find_new_position(
                                row_index, column_index, first_index
                            )
                            if (
                                (0 <= x < RESOLUTION)
                                and (0 <= y <= RESOLUTION)
                                and ((x, y) not in done_indeces)
...
lochness = [
    ...
    "O....OO....OO....OOO",
    ".O..O..O..O..O..O...",
]  # sea monster interpretation - O is important
lochs_vals_count = [
    lochness[0].count("O"),
    lochness[1].count("O"),
    lochness[2].count("O"),
            ...
            if new_image_positions[key][0] == (x, y):  # find image for correct position
                if new_line is None:
                    ...
                    new_line = np.concatenate(
                        (new_line, current), axis=1
                    )  # concatenate neighbours
                break
    if whole_image is None:
        whole_image = new_line
    else:
        whole_image = np.concatenate(
            (whole_image, new_line), axis=0
        )  # concatenate neighbours on 0. axis
